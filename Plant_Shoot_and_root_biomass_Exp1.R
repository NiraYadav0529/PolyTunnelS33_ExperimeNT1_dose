...
Plant_traits <- Plant_traits %>%
  mutate(
    ...
    Fertilizer_Type = factor(Fertilizer.type),                     # Convert Fertilizer.type to Fertilizer_Type
...
m1_shoot <- lm(Fresh.Biomass_harvestday..gm. ~ Dose * Fertilizer_Type * Plant.type, data = Plant_traits)
summary(m1_shoot)
...
pairwise_comparisons_shoot <- emmeans(m1_shoot, ~ Dose | Plant.type)
letters_shoot <- cld(pairwise_comparisons_shoot) %>%
  as.data.frame()

# Merge the letters for shoot biomass with biomass_summary using Dose and Plant.type
biomass_summary_shoot <- biomass_summary %>%
  left_join(letters_shoot, by = c("Dose", "Plant.type"))
